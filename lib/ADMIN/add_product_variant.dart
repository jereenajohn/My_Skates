...
class _variantState extends State<variant> {
  ...
  Future<void> loadAllData() async {
    ...
    await Future.wait([getVariants(), getvalues(), getattribute()]);
  ...
  void _handleUpdateProduct(Map<String, dynamic> product) {
    ...
    Navigator.push(
      context,
      slideRightToLeftRoute(UpdateProductVariant(productId: product['id'])),
    );
  }
    ...
    try {
      ...
      selectedAttributes.forEach((attrId, valueIds) {
        if (valueIds.isNotEmpty) {
          attributesPayload[attrId.toString()] = valueIds
              .map((v) => int.parse(v))
              .toList();
        }
      });
                  ...
                  Expanded(
                    child: ListView(
                      children: values.map<Widget>((item) {
                        ...
                        final isSelected =
                            selectedAttributes[attributeId]?.contains(
                              valueId,
                            ) ??
                            false;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            v['sku']?.isNotEmpty == true ? v['sku'] : "Variant #${v['id']}",
            maxLines: 1,
  ...
  void _addAttributeWithValues() {
    ...
    setState(() {
      selectedAttributes[activeAttributeId!] = List.from(tempSelectedValueIds);
