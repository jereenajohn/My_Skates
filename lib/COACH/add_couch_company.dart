...
class AdminCoachCompaniesPage extends StatefulWidget {
  const AdminCoachCompaniesPage({super.key});

  @override
  State<AdminCoachCompaniesPage> createState() =>
      _AdminCoachCompaniesPageState();
}

class _AdminCoachCompaniesPageState extends State<AdminCoachCompaniesPage>
    with SingleTickerProviderStateMixin {
  // ── Data ──────────────────────────────────────────────────────────────
  List<CoachCompany> _coachCompanies = [];
  ...
  // ───────────────────────────────────────────────────────────────────────
  // API METHODS FOR COACH COMPANIES
  // ───────────────────────────────────────────────────────────────────────

  Future<void> _fetchCoachCompanies() async {
    setState(() {
      _isLoading = true;
      _error = null;
    });
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$api/api/myskates/coach/companies/view/'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      print("COACH COMPANIES GET STATUS: ${response.statusCode}");
      print("COACH COMPANIES GET RESPONSE: ${response.body}");
      ...
      if (response.statusCode == 200) {
        ...
        List<CoachCompany> companies = [];
        for (var item in companiesList) {
          if (item is Map<String, dynamic>) {
            companies.add(CoachCompany.fromJson(item));
          }
        }

        setState(() {
          _coachCompanies = companies;
          _isLoading = false;
        });
        _animController.forward(from: 0);
      } else {
        setState(() {
          _error = 'Failed to load coach companies: ${response.statusCode}';
          _isLoading = false;
        });
      }
    } catch (e) {
      print("Error fetching coach companies: $e");
      setState(() {
        _error = e.toString();
        _isLoading = false;
      });
  ...
  Future<void> _createCoachCompany() async {
    ...
    try {
      ...
      print("COACH COMPANY POST STATUS: ${response.statusCode}");
      print("COACH COMPANY POST RESPONSE: ${response.body}");

      if (response.statusCode == 200 || response.statusCode == 201) {
        _clearForm();
        await _fetchCoachCompanies();
        _showSnackBar('Coach company added successfully');
        _cancelEdit();
      } else {
        _showSnackBar(
  ...
  Future<void> _updateCoachCompany() async {
    ...
    try {
      ...
      final response = await http.put(
        Uri.parse(
          '$api/api/myskates/coach/companies/update/${_editingCompany!.id}/',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
      ...
      print("COACH COMPANY PUT STATUS: ${response.statusCode}");
      print("COACH COMPANY PUT RESPONSE: ${response.body}");

      if (response.statusCode == 200 || response.statusCode == 201) {
        _cancelEdit();
        await _fetchCoachCompanies();
        _showSnackBar('Coach company updated successfully');
      } else {
        _showSnackBar(
          _extractError(response.body, response.statusCode),
          isError: true,
  ...
  Future<void> _deleteCoachCompany(int id) async {
    setState(() {
      _isDeleting = true;
      _deletingId = id;
    });
    ...
    try {
      ...
      final response = await http.delete(
        Uri.parse('$api/api/myskates/coach/companies/update/$id/'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      print("COACH COMPANY DELETE STATUS: ${response.statusCode}");

      if (response.statusCode == 200 || response.statusCode == 204) {
        await _fetchCoachCompanies();
        _showSnackBar('Coach company deleted successfully');
      } else {
        _showSnackBar(
          _extractError(response.body, response.statusCode),
          isError: true,
  ...
  void _startAddNew() {
    ...
    if (_coachCompanies.isNotEmpty) {
      _showSnackBar(
        'You can only add one company. Please edit the existing company.',
        isError: true,
      );
      return;
    }
  ...
  void _startEdit(CoachCompany company) {
  ...
  void _showDeleteDialog(CoachCompany company) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              _deleteCoachCompany(company.id);
            },
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          // Only show add button if no company exists
          if (!_isLoading &&
              !_isAddingNew &&
              _editingCompany == null &&
              _coachCompanies.isEmpty)
            IconButton(
              icon: const Icon(
                Icons.add_rounded,
          ...
          children: [
            Row(
              children: [
                ...
                Text(
                  isEditing ? 'Edit My Company' : 'Add My Company',
            ...
            // Info message for single company limit
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'You can only have one company. You can edit or delete your existing company.',
                      style: TextStyle(
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isSaving
                    ? null
                    : (isEditing ? _updateCoachCompany : _createCoachCompany),
                style: ElevatedButton.styleFrom(
  ...
  Widget _buildCoachCompanyCard(CoachCompany company, int index) {
    return AnimatedContainer(
      duration: const Duration(milliseconds: 300),
      ...
      child: _glassWrap(
        ...
        child: Column(
          ...
          children: [
            ...
            if (company.createdBy != null && company.createdBy!.isNotEmpty) ...[
      ...
      child: Column(
        children: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Add My Company'),
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.tealAccent,
              foregroundColor: Colors.black,
              shape: RoundedRectangleBorder(
    ...
    return _glassWrap(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          ElevatedButton.icon(
            onPressed: _fetchCoachCompanies,
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_isLoading)
                        ...
                        _buildErrorState()
                      else if (_coachCompanies.isEmpty)
                        _buildEmptyState()
                      else
                        FadeTransition(
                          opacity: _fadeAnim,
                          child: SlideTransition(
                            ...
                            child: Column(
                              children: [
                                for (int i = 0; i < _coachCompanies.length; i++)
                                  _buildCoachCompanyCard(_coachCompanies[i], i),
                              ],
                            ),
                          ),
                        ),
...
class CoachCompany {
  ...
  factory CoachCompany.fromJson(Map<String, dynamic> json) {
    return CoachCompany(
      id: json['id'] as int,
