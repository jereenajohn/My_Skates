...
class _UserFollowersListState extends State<UserFollowersList> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          filteredFollowers = List<Map<String, dynamic>>.from(normalized);
  ...
  void filterFollowers(String query) {
    ...
    if (q.isEmpty) {
      setState(() {
        filteredFollowers = followers.cast<Map<String, dynamic>>();
      });
      return;
    }

    setState(() {
      filteredFollowers = followers.cast<Map<String, dynamic>>().where((
        follower,
      ) {
        final firstName = (follower["first_name"] ?? "")
            .toString()
            .toLowerCase();
        final lastName = (follower["last_name"] ?? "").toString().toLowerCase();
        final userType = (follower["user_type"] ?? "").toString().toLowerCase();
